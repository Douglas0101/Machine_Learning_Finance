...
"""
Correções necessárias para os transformadores personalizados no build_features.py.

As principais alterações são:
1. Adicionar método get_feature_names_out() em todos os transformadores personalizados
2. Adicionar __sklearn_is_fitted__ para compatibilidade com verificação de ajuste
3. Melhorar o método _extract_feature_names() para maior robustez
"""


# 1. Adicionar get_feature_names_out ao OutlierHandler
...
# 2. Adicionar get_feature_names_out ao WoETransformer
...
# 3. Adicionar get_feature_names_out ao AgeCalculator
class AgeCalculator(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y: Optional[pd.Series] = None) -> 'AgeCalculator':
        """Não faz nada, apenas retorna self."""
        self._fitted = True
        self.input_features_ = X.columns.tolist()
        return self
    ...
    def get_feature_names_out(self, input_features=None):
        ...
        """
        if not self._fitted:
            raise NotFittedError("AgeCalculator não foi treinado. Chame fit() antes de get_feature_names_out().")

        # Se input_features não for fornecido, usar as colunas armazenadas
        if input_features is None:
            if hasattr(self, 'input_features_'):
                # Adicionar a coluna de saída ao conjunto de colunas de entrada
                ...
                # Se não temos colunas armazenadas, retornar pelo menos a coluna de saída
                return np.array([self.output_col])

        # Adicionar a coluna de saída ao conjunto de colunas fornecidas
        all_columns = list(input_features)
        if self.output_col not in all_columns:
            all_columns.append(self.output_col)
...
# 4. Adicionar get_feature_names_out ao DateFeatureExtractor
class DateFeatureExtractor(BaseEstimator, TransformerMixin):
    ...
    def get_feature_names_out(self, input_features=None):
        ...
        """
        if not self._fitted:
            raise NotFittedError("DateFeatureExtractor não foi treinado. Chame fit() antes de get_feature_names_out().")
        ...
        # Fallback: gerar nomes das colunas
        output_features = []
...
# 5. Versão corrigida de _extract_feature_names para FeatureBuilder
...
class FeatureBuilder:
        ...
        for col in cols:
            ...
            if pd.api.types.is_categorical_dtype(df[col]) or pd.api.types.is_object_dtype(df[col]):
                ...
                if pd.api.types.is_numeric_dtype(df[col]) and df[col].nunique() <= max_categories:
                    categorical_cols.append(col)
                    logger.info(
                        f"Coluna numérica '{col}' com {df[col].nunique()} valores únicos será tratada como categórica.")
                else:
    ...
    def _create_interaction_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col1, col2 in interactions:
            ...
            # Se uma é categórica e a outra numérica
                  ...
                  (pd.api.types.is_categorical_dtype(result[col2]) or pd.api.types.is_object_dtype(result[col2]))):
                ...
                for category in result[col2].dropna().unique():
                  ...
                  (pd.api.types.is_categorical_dtype(result[col1]) or pd.api.types.is_object_dtype(result[col1]))):
                # Criar uma feature por categoria
            ...
            elif ((pd.api.types.is_categorical_dtype(result[col1]) or pd.api.types.is_object_dtype(result[col1])) and
                  (pd.api.types.is_categorical_dtype(result[col2]) or pd.api.types.is_object_dtype(result[col2]))):
    ...
    """
    Correções para os métodos fit, transform e fit_transform na classe FeatureBuilder.

    Estas implementações são mais robustas para lidar com diferentes tipos de transformadores
    e garantir a compatibilidade com a API do scikit-learn.
    """
    ...
    def fit(self, X: pd.DataFrame, y: Optional[pd.Series] = None) -> 'FeatureBuilder':
        ...
        if 'dates' in self.transformers:
            try:
                ...
                logger.info(
                    f"Transformador de datas aplicado: {len(self.transformers['dates']['columns'])} colunas processadas")
            except Exception as e:
        ...
        # Extrair nomes das features (método corrigido)
        try:
        ...
        if isinstance(final_data, np.ndarray):
            try:
                ...
                if self.feature_names and len(self.feature_names) == final_data.shape[1]:
                    ...
                    logger.warning(
                        f"Incompatibilidade no número de features: {len(self.feature_names)} nomes vs {final_data.shape[1]} colunas")
                    result = pd.DataFrame(
                        final_data,
    ...
    def get_feature_importances(self, model) -> pd.DataFrame:
        ...
        if len(importances) != len(self.feature_names):
            logger.warning(
                f"Número de importâncias ({len(importances)}) difere do número de features ({len(self.feature_names)}).")
...
def main():
    ...
    if args.data_path:
        ...
        X_date = pd.DataFrame({
            'data_cadastro': dates,
            'data_nascimento': [pd.Timestamp('1970-01-01') + pd.Timedelta(days=np.random.randint(365 * 18, 365 * 70))
                                for _ in range(1000)]
        })
